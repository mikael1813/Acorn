...
{
	SDL_Rect srcRect = { tileSize * x,tileSize * y,tileSize,tileSize };

	Vector2D camera = Camera::GetInstance()->GetPosition();

	SDL_Rect dstRect = { tileSize * row - camera.X,tileSize * frame - camera.Y,tileSize,tileSize };
	//SDL_Rect dstRect = { tileSize * frame,tileSize * row,tileSize,tileSize };
